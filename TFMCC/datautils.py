...
def load_UCR(dataset):
    ...
    #代码提取所有唯一标签，并将它们映射到整数（从 0 开始
    labels = np.unique(train_array[:, 0])
    transform = {}
    for i, l in enumerate(labels):
        transform[l] = i

    #从 UCR 数据集中提取和处理训练和测试数据
    ...
    #对于其他数据集，则计算整个训练集的均值和标准差，对训练和测试数据进行标准化
...
def load_UEA(dataset):
    with open(f'datasets/UEA/{dataset}/{dataset}_TRAIN.arff', 'r', encoding='utf-8') as f:
        # train_data = loadarff(f'datasets/UEA/{dataset}/{dataset}_TRAIN.arff')[0]
        # test_data = loadarff(f'datasets/UEA/{dataset}/{dataset}_TEST.arff')[0]
        train_data = loadarff(f)[0]
    with open(f'datasets/UEA/{dataset}/{dataset}_TEST.arff', 'r', encoding='utf-8') as f:
        test_data = loadarff(f)[0]
